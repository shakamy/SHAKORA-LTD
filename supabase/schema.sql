...
-- SHAKS CMS V1.0.0 security baseline
-- Authenticated CMS access is restricted to users in admin_users.
...
-- Remove the development-era blanket authenticated policies.
...
create policy admin_projects on public.projects for all to authenticated using(public.is_admin()) with check(public.is_admin());
...
create policy admin_services on public.services for all to authenticated using(public.is_admin()) with check(public.is_admin());
create policy admin_technologies on public.technologies for all to authenticated using(public.is_admin()) with check(public.is_admin());
create policy admin_project_tech on public.project_technologies for all to authenticated using(public.is_admin()) with check(public.is_admin());
create policy admin_project_services on public.project_services for all to authenticated using(public.is_admin()) with check(public.is_admin());
create policy admin_media on public.media for all to authenticated using(public.is_admin()) with check(public.is_admin());
create policy admin_project_media on public.project_media for all to authenticated using(public.is_admin()) with check(public.is_admin());
create policy admin_testimonials on public.testimonials for all to authenticated using(public.is_admin()) with check(public.is_admin());
create policy admin_posts on public.posts for all to authenticated using(public.is_admin()) with check(public.is_admin());
create policy admin_messages on public.contact_messages for all to authenticated using(public.is_admin()) with check(public.is_admin());
create policy admin_settings on public.website_settings for all to authenticated using(public.is_admin()) with check(public.is_admin());
create policy admin_seo on public.seo_settings for all to authenticated using(public.is_admin()) with check(public.is_admin());
create policy admin_navigation on public.navigation for all to authenticated using(public.is_admin()) with check(public.is_admin());
create policy admin_activity on public.admin_activity for all to authenticated using(public.is_admin()) with check(public.is_admin());
drop policy if exists "authenticated project categories manage" on public.project_categories;
create policy admin_project_categories on public.project_categories for all to authenticated using(public.is_admin()) with check(public.is_admin());
